...
if (tg.initDataUnsafe && tg.initDataUnsafe.user) {
    ...
    // Проверяем, админ ли вошел
    if (userData.id === ADMIN_ID) {
        isAdmin = true;
        isSubscribedUser = true; // Админу подписка активна по умолчанию!
        console.log("👑 Добро пожаловать, Создатель!");
    }
}
tg.expand();
...
function initMap3D() {
    ...
    const clusters = [
        { minX: 450, maxX: 870, minY: 80, maxY: 140 },  // Евразия
        { minX: 80,  maxX: 280, minY: 75, maxY: 150 },  // Северная Америка
        { minX: 240, maxX: 330, minY: 240, maxY: 380 }, // Южная Америка
        { minX: 450, maxX: 560, minY: 190, maxY: 340 }, // Африка
        { minX: 620, maxX: 850, minY: 150, maxY: 250 }, // Азия
        { minX: 760, maxX: 870, minY: 310, maxY: 390 }  // Австралия
    ];
...
// 🎁 6. ИНТЕРАКТИВНАЯ ПЛАШКА ПРОМОКОДА
...
function applyPromoCode() {
    ...
    if (val === "SOL2025" || val === "VIP") {
        ...
        document.getElementById("sub-days-left").innerText = "28 дней";
...
// 🔐 ПРОВЕРКА КАТЕГОРИИ ИГР (Блокировка без подписки)
function checkGameLockStatus() {
    const lockScreen = document.getElementById("game-lock-screen");
    const isVisible = lockScreen && !lockScreen.classList.contains("hidden");
...
// 🎮 8. КОСМИЧЕСКАЯ ИГРА: ОХОТА НА КОМЕТЫ (ВЕРСИЯ 2.0)
...
function determineLoot(x, y) {
    ...
    if (roll < 0.40) {
        // 🟢 Зеленый дроп: + Рефералка
        const addPercent = parseFloat((0.5 + Math.random() * 1.5).toFixed(1));
        currentRefPercent = Math.min(maxRefPercent, currentRefPercent + addPercent);
        
        updateRefUI();
        createFloatingText(x, y, `+${addPercent}%`, 'green-text');
        hint.innerText = `💫 Комета расколота: УСПЕХ! +${addPercent}% к рефералке!`;

    } else if (roll < 0.65) {
        // 🔴 Красный дроп: - Рефералка
        const subPercent = parseFloat((0.3 + Math.random() * 1.0).toFixed(1));
        currentRefPercent = Math.max(5.0, currentRefPercent - subPercent);
        
        updateRefUI();
        createFloatingText(x, y, `-${subPercent}%`, 'red-text');
        hint.innerText = `☄️ Комета взорвалась: КРИТ! Процент рефералки снижен на -${subPercent}%`;

    } else if (roll < 0.85) {
        // 🟡 Желтый дроп: 0% пустышка
        createFloatingText(x, y, `0%`, 'yellow-text');
        hint.innerText = `🕳️ Из кометы выпала космическая пыль. Никаких бонусов.`;

    } else {
        // 🎡 ВЫБИВАНИЕ СКИДКИ + ЗАПУСК КОЛЕСА ФОРТУНЫ!
        const randomDiscount = Math.floor(10 + Math.random() * 21); // От 10% до 30%
        createFloatingText(x, y, `🔥 -${randomDiscount}%!`, 'green-text');
        hint.innerText = `🔥 ВАУ! Выбита скидка -${randomDiscount}%. Крутим колесо тарифов!`;
        
        setTimeout(() => {
            spinTariffWheel(randomDiscount);
        }, 1200);
    }
}
...
function spinTariffWheel(discountValue) {
    ...
    anime({
        ...
        complete: () => {
            ...
            hint.innerText = `🎉 СКИДКА ${discountValue}% успешно применена на тариф ${targetSegment} мес.!`;
            if (tg.HapticFeedback) tg.HapticFeedback.notificationOccurred("success");
...
const faqAnswers = {
    ...
    5: "📈 Для увеличения процента реферальной программы перейдите во вкладку 'Игры', запускайте сканирование радара и ловите скоростные кометы! Каждая пойманная комета дает до +1.5% к вашей ставке.",
...
function animateDipText(newText) {
    ...
    if (currentWords.length > 0) {
        currentWords.forEach((word, idx) => {
            setTimeout(() => {
                word.classList.add("dip-shrink");
            }, idx * 12);
        });

        setTimeout(() => {
            renderNewWordsWithExpand(newText);
        }, currentWords.length * 12 + 160);
    } else {
        renderNewWordsWithExpand(newText);
    }
}
...
function renderNewWordsWithExpand(text) {
    ...
    words.forEach((word, idx) => {
        ...
        setTimeout(() => {
            span.classList.remove("dip-shrink");
        }, 40 + idx * 20);
    });
}
...
function selectTariff(months) {
    // В тестовом режиме бесконечность (значок ∞)
...
function switchNav(index, screenId) {
    ...
    if (screenId === 'info') {
        setTimeout(() => {
            animateDipText(defaultInfoText);
        }, 100);
    }
...
window.onload = () => {
    ...
    // Если зашел админ — сразу даем ему фулл доступ без покупки тарифов
    if (isAdmin) {
        isSubscribedUser = true;
        
        // Меняем плашку подписки на "Создатель" со значком бесконечности ∞
        ...
        if (progressBar) {
            ...
            progressBar.style.background = "linear-gradient(90deg, #f59e0b, #a855f7)"; // Золотисто-фиолетовый градиент для админа
        }
        
        // Разблокируем игры автоматически
        checkGameLockStatus();
    }
};